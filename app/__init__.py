...
# Resolve repository root and static folder (static/ at project root)
...
# If FLASK_LIMITER_STORAGE_URI or RATELIMIT_STORAGE_URL is set (e.g., redis://localhost:6379), use it for storage;
# otherwise fall back to the default in-memory storage (warning emitted by flask-limiter).
storage_uri = os.environ.get('FLASK_LIMITER_STORAGE_URI') or os.environ.get('RATELIMIT_STORAGE_URL')
if storage_uri:
    limiter = Limiter(key_func=get_remote_address, app=app, default_limits=[], storage_uri=storage_uri)
else:
...
# The old file had a guarded __main__ that did environment-sensitive binding.
# Keep that logic available through `run()` so the top-level launcher or module
# invocation can run the server the same way.
...
# When used as a package entrypoint, prefer calling run_from_env explicitly.
